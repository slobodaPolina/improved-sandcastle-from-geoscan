...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		 ...
		 * ).and().formLogin()
		 * .loginPage("/login").loginProcessingUrl("/login").permitAll().and().csrf().
		 * disable(); super.configure(http);
		 */
		http.authorizeRequests().antMatchers("/hello").permitAll().and().formLogin().loginPage("/login").and().logout()
				.and().csrf().disable();
